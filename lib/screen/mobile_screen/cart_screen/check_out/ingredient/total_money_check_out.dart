      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            cost_text_line(title: 'Items (' + widget.order.productList.length.toString() + ')', content: getStringNumber(calculatetotalMoney()) + ' .vnđ', size: width/25, contentColor: Colors.black, titleColor: Colors.grey,),

            SizedBox(height: 10,),

            GestureDetector(
              child: cost_text_line(title: 'Voucher', content: (widget.order.voucher.id == '' ? 'Chưa chọn mã' : ('- ' + getStringNumber(getVoucherSale(widget.order.voucher, calculatetotalMoney())) + ' .vnđ')), size: width/25, contentColor: Colors.blue, titleColor: Colors.grey,),
              onTap: () {
            ...
            cost_text_line(title: 'Tổng tiền', content: getStringNumber(calculatetotalMoney() - getVoucherSale(widget.order.voucher, calculatetotalMoney())) + ' .vnđ', size: width/25, contentColor: Colors.black, titleColor: Colors.black,),
                  ...
                  onPressed: () async {
                    if (_canPush()) {
                      ...
                      await check_out_controller.push_new_order(
                        widget.order,
                            () {setState(() {loading = false;}); Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => finalData.isLogin ? main_screen() : preview_screen()),);},
